...
import com.github.hermod.ser.impl.IndexedObjectsMsg;
import com.github.hermod.ser.impl.Msgs;

/**
 * <p>DirectBytesHermodMsgSerializer.</p>
 * 
 * @author anavarro - Jan 6, 2014
 * 
 */
public final class DirectBytesHermodMsgSerializer implements BytesMsgSerializer, ByteBufferMsgSerializer {

    private static final DirectBytesHermodMsgSerializer DIRECT_BYTES_HERMOD_MSG_SERIALIZER = new DirectBytesHermodMsgSerializer();
     ...
     * @return
     */
    public static DirectBytesHermodMsgSerializer create() {
        return new DirectBytesHermodMsgSerializer();
    }
     ...
     * @return
     */
    public static DirectBytesHermodMsgSerializer get() {
        return DIRECT_BYTES_HERMOD_MSG_SERIALIZER;
    }
     ...
     */
    //TODO to change implementation with a DirectBytes
    @Override
    public void deserializeFromBytes(final byte[] aSrcBytes, final int aSrcOffset, final int aSrcLength, Msg aDestMsg) {
        if (aDestMsg.isBytesSerializable()) {
            ...
            while (pos < aSrcOffset + aSrcLength) {
                if ((aSrcBytes[pos] & TYPE_MASK) == SKIPPED_KEYS_TYPE) {
                    final int lengthMask = aSrcBytes[pos++] & LENGTH_MASK;
                    key += (((lengthMask < LENGTH_ENCODED_IN_AN_UNSIGNED_BYTE) ? lengthMask
                    : (lengthMask == LENGTH_ENCODED_IN_AN_UNSIGNED_BYTE) ? (XFF & aSrcBytes[pos++]) : (aSrcBytes[pos++] & XFF)
                    | ((aSrcBytes[pos++] & XFF) << EIGHT) | ((aSrcBytes[pos++] & XFF) << SIXTEEN) | ((aSrcBytes[pos++] & XFF) << TWENTY_FOUR))) + 1;
                } else {
                    final int lengthMask = aSrcBytes[pos++] & LENGTH_MASK;
                    // TODO to optimize
                    pos += (((lengthMask < LENGTH_ENCODED_IN_AN_UNSIGNED_BYTE) ? lengthMask
                    : (lengthMask == LENGTH_ENCODED_IN_AN_UNSIGNED_BYTE) ? XFF & aSrcBytes[pos]
                    : ((aSrcBytes[pos] & XFF) | ((aSrcBytes[pos + ONE] & XFF) << EIGHT) | ((aSrcBytes[pos + TWO] & XFF) << SIXTEEN) | ((aSrcBytes[pos
            ...
            while (pos < aSrcOffset + aSrcLength) {
                ...
                if ((type & TYPE_MASK) == SKIPPED_KEYS_TYPE) {
                    ...
                    & aSrcBytes[pos++] : (aSrcBytes[pos++] & XFF) | ((aSrcBytes[pos++] & XFF) << EIGHT) | ((aSrcBytes[pos++] & XFF) << SIXTEEN)
                    | ((aSrcBytes[pos++] & XFF) << TWENTY_FOUR)) + 1;
                }
                // Decode values
                else {
                    switch (type) {
                    //  All fixed type
                        case BYTE_TYPE:
                            aDestMsg.set(key, (byte) ((aSrcBytes[pos++] & XFF)));
                            break;

                        case SHORT_TYPE:
                            aDestMsg.set(key, (short) ((((short) aSrcBytes[pos++] & XFF)) | (((short) aSrcBytes[pos++]) << EIGHT)));
                            break;

                        case INT_TYPE:
                            aDestMsg.set(key, (((int) aSrcBytes[pos++] & XFF)) | (((int) aSrcBytes[pos++] & XFF) << EIGHT)
                            | (((int) aSrcBytes[pos++] & XFF) << SIXTEEN) | (((int) aSrcBytes[pos++]) << TWENTY_FOUR));
                            break;
                        ...
                        case DOUBLE_TYPE:
                            aDestMsg.set(
                            key,
                            Double.longBitsToDouble((((long) aSrcBytes[pos++] & XFF) | (((long) aSrcBytes[pos++] & XFF) << EIGHT)
                            | (((long) aSrcBytes[pos++] & XFF) << SIXTEEN) | (((long) aSrcBytes[pos++] & XFF) << TWENTY_FOUR)
                            | (((long) aSrcBytes[pos++] & XFF) << THIRTY_TWO) | (((long) aSrcBytes[pos++] & XFF) << FORTY)
                            | (((long) aSrcBytes[pos++] & XFF) << FORTY_EIGHT) | (((long) aSrcBytes[pos++] & XFF) << FIFTY_SIX))));
                            break;

                        case FIVE_BITS_DECIMAL_TYPE:
                            aDestMsg.set(
                            key,
                            Double.longBitsToDouble((((long) aSrcBytes[pos++] & XFF) << EIGHT) | (((long) aSrcBytes[pos++] & XFF) << SIXTEEN)
                            | (((long) aSrcBytes[pos++] & XFF) << TWENTY_FOUR) | (((long) aSrcBytes[pos++] & XFF) << THIRTY_TWO)),
                            ((byte) aSrcBytes[pos++] & XFF));
                            break;

                        case FLOAT_TYPE:
                            aDestMsg.set(
                            key,
                            Float.intBitsToFloat((((int) aSrcBytes[pos++] & XFF)) | (((int) aSrcBytes[pos++] & XFF) << EIGHT)
                            | (((int) aSrcBytes[pos++] & XFF) << SIXTEEN) | (((int) aSrcBytes[pos++] & XFF) << TWENTY_FOUR)));
                            break;

                        case LONG_TYPE:
                            aDestMsg.set(key, (long) ((((long) aSrcBytes[pos++] & XFF)) | (((long) aSrcBytes[pos++] & XFF) << EIGHT)
                            | (((long) aSrcBytes[pos++] & XFF) << SIXTEEN) | (((long) aSrcBytes[pos++] & XFF) << TWENTY_FOUR)
                            | (((long) aSrcBytes[pos++] & XFF) << THIRTY_TWO) | (((long) aSrcBytes[pos++] & XFF) << FORTY)
                            | (((long) aSrcBytes[pos++] & XFF) << FORTY_EIGHT) | (((long) aSrcBytes[pos++]) << FIFTY_SIX)));
                            break;
                        ...
                        default:
                            ...
                            : (lengthMask == LENGTH_ENCODED_IN_AN_UNSIGNED_BYTE) ? XFF & aSrcBytes[pos++] : (aSrcBytes[pos++] & XFF)
                            | ((aSrcBytes[pos++] & XFF) << EIGHT) | ((aSrcBytes[pos++] & XFF) << SIXTEEN) | ((aSrcBytes[pos++] & XFF) << TWENTY_FOUR);
                            if (lengthMask != 0) {
                                switch (typeMask) {
                                    ...
                                    case STRING_UTF_8_TYPE:
                                        aDestMsg.set(key, new String(aSrcBytes, pos, fieldLength, UTF_8_CHARSET));
                                        pos += fieldLength;
                                        break;

                                    case MSG_TYPE:
                                        final Msg msg = IndexedObjectsMsg.create();
                                        this.deserializeFromBytes(aSrcBytes, pos, fieldLength, msg);
                                        pos += fieldLength;
                                    ...
                                    case ARRAY_FIXED_VALUE_TYPE:
                                        final byte arrayType = aSrcBytes[pos++];
                                        final int fixedArrayLength = fieldLength - ONE;
                                        // For Fixed typed
                                        switch (arrayType) {
                                            case BYTE_TYPE:
                                                final byte[] byteArray = new byte[fixedArrayLength];
                                                System.arraycopy(aSrcBytes, pos, byteArray, 0, fixedArrayLength);
                                            ...
                                            case SHORT_TYPE:
                                                ...
                                                for (int i = 0; i < shorts.length; i++) {
                                                    shorts[i] = (short) ((((short) aSrcBytes[pos++] & XFF)) | (((short) aSrcBytes[pos++] & XFF) << EIGHT));
                                                }
                                            ...
                                            case INT_TYPE:
                                                ...
                                                for (int i = 0; i < ints.length; i++) {
                                                    ints[i] = (((int) aSrcBytes[pos++] & XFF)) | (((int) aSrcBytes[pos++] & XFF) << EIGHT)
                                                    | (((int) aSrcBytes[pos++] & XFF) << SIXTEEN) | (((int) aSrcBytes[pos++] & XFF) << TWENTY_FOUR);
                                                }
                                            ...
                                            case DOUBLE_TYPE:
                                                ...
                                                for (int i = 0; i < doubles.length; i++) {
                                                    doubles[i] = Double
                                                    .longBitsToDouble((((long) aSrcBytes[pos++] & XFF) | (((long) aSrcBytes[pos++] & XFF) << EIGHT)
                                                    | (((long) aSrcBytes[pos++] & XFF) << SIXTEEN) | (((long) aSrcBytes[pos++] & XFF) << TWENTY_FOUR)
                                                    | (((long) aSrcBytes[pos++] & XFF) << THIRTY_TWO) | (((long) aSrcBytes[pos++] & XFF) << FORTY)
                                                    | (((long) aSrcBytes[pos++] & XFF) << FORTY_EIGHT) | (((long) aSrcBytes[pos++] & XFF) << FIFTY_SIX)));
                                                }
                                            ...
                                            case LONG_TYPE:
                                                ...
                                                for (int i = 0; i < longs.length; i++) {
                                                    longs[i] = (((long) aSrcBytes[pos++] & XFF)) | (((long) aSrcBytes[pos++] & XFF) << EIGHT)
                                                    | (((long) aSrcBytes[pos++] & XFF) << SIXTEEN) | (((long) aSrcBytes[pos++] & XFF) << TWENTY_FOUR)
                                                    | (((long) aSrcBytes[pos++] & XFF) << THIRTY_TWO) | (((long) aSrcBytes[pos++] & XFF) << FORTY)
                                                    | (((long) aSrcBytes[pos++] & XFF) << FORTY_EIGHT)
                                                    | (((long) aSrcBytes[pos++] & XFF) << FIFTY_SIX);
                                                }
                                            ...
                                            case FLOAT_TYPE:
                                                ...
                                                for (int i = 0; i < floats.length; i++) {
                                                    floats[i] = Float.intBitsToFloat((((int) aSrcBytes[pos++] & XFF))
                                                    | (((int) aSrcBytes[pos++] & XFF) << EIGHT) | (((int) aSrcBytes[pos++] & XFF) << SIXTEEN)
                                                    | (((int) aSrcBytes[pos++] & XFF) << TWENTY_FOUR));
                                                }
                                    ...
                                    case ARRAY_VARIABLE_VALUE_TYPE:
                                        ...
                                        if (lengthMask != 0) {
                                            ...
                                            this.deserializeFromBytes(aSrcBytes, pos, fieldLength, arrayAsMsg);
                                            pos += fieldLength;
  ...
  //TODO to change implementation with a DirectBytes
    @Override
    public int serializeToBytes(final Msg aSrcMsg, byte[] aDestBytes, final int aDestOffset) {
  ...
  //TODO to change implementation with a DirectBytes
     ...
     */
    @Override
    public void serializeToByteBuffer(final Msg aSrcMsg, ByteBuffer aDestByteBuffer) {
     ...
     */
    @Override
    public void deserializeFromByteBuffer(final ByteBuffer aSrcByteBuffer, Msg aDestMsg) {
